...
void ResponseParser::ParseStdout(const_iter &iter, long &length, long ct_len, long pd_len)
{
  ...
  if (!in_content_) {
    if ((pos = str.find("\r\n\r\n")) != std::string::npos) {
      ...
      std::string k, v, kv_str(iter, iter + pos + 2);
      while ((epos = kv_str.find("\r\n", bpos)) != std::string::npos) {
        std::string temp = kv_str.substr(bpos, epos - bpos);
        k = std::string(temp, 0, temp.find(':'));
        v = std::string(temp, temp.find(':') + 2, temp.size());
        name_value_data_.emplace(k, v);
        bpos = epos + 2;
      }
  ...
  auto len2 = pd_len > length ? length : pd_len;                  // pending data
  iter += len2;
  length -= len2;
  pd_len -= len2;
