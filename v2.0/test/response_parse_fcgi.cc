...
void func()
{
  ...
  auto connect = platinum::GetCurrentServer()->NewConnector(addr, platinum::FCGI);
  connect->SetWriteCallback([=]() {
    std::cout << "Start send to fpm" << std::endl;
    connect->SendData((char *)(&b), sizeof(b));
    for (const auto &var : p) {
      connect->SendData(reinterpret_cast<char *>(var.first.get()), var.second);
    }
    connect->SendData(reinterpret_cast<char *>(empty_p.first.get()), empty_p.second);
    for (const auto &var : i) {
      connect->SendData(reinterpret_cast<char *>(var.first.get()), var.second);
    }
    connect->SendData(reinterpret_cast<char *>(empty_i.first.get()), empty_i.second);
